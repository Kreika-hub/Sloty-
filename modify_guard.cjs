...
// Normalize line endings in the file to simple \n
...
const newExitBlock = `        <div style="background:#f8f9fa; border-radius:16px; padding:15px; margin-bottom:24px; text-align:center; border:2px solid \${totalOwed > 0 ? '#e63946' : '#22c55e'};">
           <div style="font-size:0.6rem; font-weight:900; color:#999; text-transform:uppercase;">MONTO A COBRAR</div>
           <div style="font-size:2.2rem; font-weight:950; color:\${totalOwed > 0 ? '#e63946' : '#22c55e'};">$` + `\TokenOwedValFixed</div>
           \${totalOwed > 0 ? \`
        ...
        </div>`.replace('TokenOwedValFixed', '\${totalOwed.toFixed(2)}').replace(/\r\n/g, '\n');
...
if (content.includes(oldPayForm)) {
  content = content.replace(oldPayForm, newPayForm);
