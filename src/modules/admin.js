    ...
    <div style="padding:20px; padding-bottom:120px;">
      ...
      <div style="display:grid; gap:15px; margin-top:35px;">
        ${state.levels.map(l => {
          ...
          const paletteColors = ['#1a1a2e','#e63946','#22c55e','#3b82f6','#a855f7'].map(c =>
          ...
          return '<div style="background:white; border-radius:28px; overflow:hidden; border:1.5px solid #f0f0f0; box-shadow:0 10px 30px rgba(0,0,0,0.02); position:relative;">' +
            '<div style="height:6px; background:' + cardColor + '; width:100%;"></div>' +
            '<div style="padding:20px; display:flex; justify-content:space-between; align-items:center;">' +
              '<div style="flex:1;">' + headerInner + '</div>' +
              '<div style="display:flex; align-items:center; gap:8px;">' +
                '<div style="display:flex; align-items:center; position:relative;">' +
                  '<div style="display:' + (openPaletteLevel === l.name ? 'flex' : 'none') + '; gap:4px; padding:6px; background:#f4f4f4; border-radius:12px; margin-right:8px; border:1px solid #eee; position:absolute; right:100%; top:50%; transform:translateY(-50%); z-index:10;">' +
                    paletteColors +
                  '</div>' +
                  '<button data-action="TOGGLE_PALETTE" data-name="' + l.name + '" style="background:none; border:none; color:' + (openPaletteLevel===l.name?'var(--primary)':'#bbb') + '; width:22px; height:22px; cursor:pointer;">' + ICONS.PALETTE + '</button>' +
                '</div>' +
                ...
                '</button>' +
                '<button data-action="DELETE_LEVEL" data-name="' + l.name + '" style="background:rgba(230,57,70,0.08); border:none; color:#e63946; cursor:pointer; width:36px; height:36px; border-radius:12px; display:flex; align-items:center; justify-content:center; margin-left:4px;">' +
