...
if (content.includes(oldVars)) {
  ...
  console.error('Variables target not found!');
}
...
if (content.includes(oldActions)) {
  ...
  console.error('Abono actions target not found!');
}

// 4. Update renderExitForm display
const oldExitAmountBlock = `        <div style="background:#f8f9fa; border-radius:16px; padding:15px; margin-bottom:24px; text-align:center; border:2px solid \${totalOwed > 0 ? '#e63946' : '#22c55e'};">
           <div style="font-size:0.6rem; font-weight:900; color:#999; text-transform:uppercase;">MONTO A COBRAR</div>
           <div style="font-size:2.2rem; font-weight:950; color:\${totalOwed > 0 ? '#e63946' : '#22c55e'};">$ExternalOwed</div>`;
// Wait, in our printout it is:
// L840:            <div style="font-size:2.2rem; font-weight:950; color:${totalOwed > 0 ? '#e63946' : '#22c55e'};">$${totalOwed.toFixed(2)}</div>
// Let's use exact string matching from lines 838 to 842:
const exactExitBlock = `        <div style="background:#f8f9fa; border-radius:16px; padding:15px; margin-bottom:24px; text-align:center; border:2px solid \${totalOwed > 0 ? '#e63946' : '#22c55e'};">
           <div style="font-size:0.6rem; font-weight:900; color:#999; text-transform:uppercase;">MONTO A COBRAR</div>
           <div style="font-size:2.2rem; font-weight:950; color:\${totalOwed > 0 ? '#e63946' : '#22c55e'};">$BlockTotal</div>`;

const exitBlockTarget = `        <div style="background:#f8f9fa; border-radius:16px; padding:15px; margin-bottom:24px; text-align:center; border:2px solid \${totalOwed > 0 ? '#e63946' : '#22c55e'};">
           <div style="font-size:0.6rem; font-weight:900; color:#999; text-transform:uppercase;">MONTO A COBRAR</div>
...
// Since escaping regex is matching exact characters, let's just do a direct simple substring replacement:
const oldExitBlockSub = `        <div style="background:#f8f9fa; border-radius:16px; padding:15px; margin-bottom:24px; text-align:center; border:2px solid \${totalOwed > 0 ? '#e63946' : '#22c55e'};">
           <div style="font-size:0.6rem; font-weight:900; color:#999; text-transform:uppercase;">MONTO A COBRAR</div>
           <div style="font-size:2.2rem; font-weight:950; color:\${totalOwed > 0 ? '#e63946' : '#22c55e'};">$${totalOwed.toFixed(2)}</div>
           \${totalOwed > 0 ? \`<div style="font-size:0.6rem; font-weight:700; color:#e63946; margin-top:4px;">EXCESO DE TIEMPO (+8H)</div>\` : \`<div style="font-size:0.6rem; font-weight:700; color:#22c55e; margin-top:4px;">CORTESÍA / PAGADO</div>\`}`;

const newExitBlockSub = `        <div style="background:#f8f9fa; border-radius:16px; padding:15px; margin-bottom:24px; text-align:center; border:2px solid \${totalOwed > 0 ? '#e63946' : '#22c55e'};">
           <div style="font-size:0.6rem; font-weight:900; color:#999; text-transform:uppercase;">MONTO A COBRAR</div>
           <div style="font-size:2.2rem; font-weight:950; color:\TokenOwedColor">$${totalOwed.toFixed(2)}</div>
           \${totalOwed > 0 ? \`
        ...
        </div>`.replace('TokenOwedColor', '${totalOwed > 0 ? \'#e63946\' : \'#22c55e\'}');

if (content.includes(oldExitBlockSub)) {
  content = content.replace(oldExitBlockSub, newExitBlockSub);
  console.log('Exit Form display updated successfully');
} else {
  console.error('Exit Form display target not found!');
  // Try dynamic match if windows CRLF differs:
  const normalizedOld = oldExitBlockSub.replace(/\r\n/g, '\n');
  const normalizedContent = content.replace(/\r\n/g, '\n');
  if (normalizedContent.includes(normalizedOld)) {
     content = normalizedContent.replace(normalizedOld, newExitBlockSub.replace(/\r\n/g, '\n'));
     console.log('Exit Form display updated with normalization');
  }
}

// 5. Update renderPaymentForm template
const oldPayForm = `  const renderPaymentForm = () => \`
...
// Let's write the new template for renderPaymentForm, inserting window.updatePayFormConversion and the Bolívares equivalents box
const newPayForm = `  const renderPaymentForm = () => {
    const rate = bcvData?.rate || 40;
...
// Clean exact content matching for oldPayForm (lines 855-890):
const oldPayFormSub = `  const renderPaymentForm = () => \`
    <div style="padding:20px; padding-bottom:120px;">
      <div style="background:white; border-radius:32px; padding:30px; box-shadow:0 15px 45px rgba(0,0,0,0.1);">
         <div style="text-align:center; margin-bottom:25px;">
            <div style="font-size:0.7rem; font-weight:900; color:#999; letter-spacing:1px; text-transform:uppercase;">DETALLE DE PAGO</div>
...
if (content.includes(oldPayFormSub)) {
  content = content.replace(oldPayFormSub, newPayForm);
  console.log('Payment form updated successfully');
} else {
  console.error('Payment form target not found!');
  const normalizedOld = oldPayFormSub.replace(/\r\n/g, '\n');
  const normalizedContent = content.replace(/\r\n/g, '\n');
  if (normalizedContent.includes(normalizedOld)) {
     content = normalizedContent.replace(normalizedOld, newPayForm.replace(/\r\n/g, '\n'));
     console.log('Payment form updated with normalization');
  }
}

// 6. Update renderSubPaymentForm (lines 940-970)
const oldSubForm = `  const renderSubPaymentForm = () => \`
     <div style="padding:20px; padding-bottom:120px;">
         <div style="background:white; border-radius:32px; padding:30px; box-shadow:0 15px 45px rgba(0,0,0,0.1);">
...
const newSubForm = `  const renderSubPaymentForm = () => {
     const rate = bcvData?.rate || 40;
...
if (content.includes(oldSubForm)) {
  content = content.replace(oldSubForm, newSubForm);
  console.log('Sub payment form updated successfully');
} else {
  console.error('Sub payment form target not found!');
  const normalizedOld = oldSubForm.replace(/\r\n/g, '\n');
  const normalizedContent = content.replace(/\r\n/g, '\n');
  if (normalizedContent.includes(normalizedOld)) {
     content = normalizedContent.replace(normalizedOld, newSubForm.replace(/\r\n/g, '\n'));
     console.log('Sub payment form updated with normalization');
  }
}

// 7. Update renderClosureSummary (Recaudo Total & Method totals)
const oldClosureTotalBox = `             <div style="background:#1a1a2e; color:white; padding:20px 10px; border-radius:24px; text-align:center;">
...
const newClosureTotalBox = `             <div style="background:#1a1a2e; color:white; padding:20px 10px; border-radius:24px; text-align:center;">
                 <div style="font-size:1.6rem; font-weight:950;">$\${total.toFixed(2)}</div>
...
if (content.includes(oldClosureTotalBox)) {
  content = content.replace(oldClosureTotalBox, newClosureTotalBox);
  console.log('Closure total box updated successfully');
} else {
  console.error('Closure total box target not found!');
  const normalizedOld = oldClosureTotalBox.replace(/\r\n/g, '\n');
  const normalizedContent = content.replace(/\r\n/g, '\n');
  if (normalizedContent.includes(normalizedOld)) {
     content = normalizedContent.replace(normalizedOld, newClosureTotalBox.replace(/\r\n/g, '\n'));
     console.log('Closure total box updated with normalization');
  }
}

const oldClosureMethodsLoop = `           <!-- METHODS -->
...
const newClosureMethodsLoop = `           <!-- METHODS -->
           <div style="background:#fdfdfd; border:1.5px solid #f8f8f8; border-radius:20px; padding:15px; margin-bottom:25px;">
              \${Object.entries(breakdown).map(([m, val]) => {
                 const rate = bcvData?.rate || 40;
...
if (content.includes(oldClosureMethodsLoop)) {
  content = content.replace(oldClosureMethodsLoop, newClosureMethodsLoop);
  console.log('Closure methods updated successfully');
} else {
  console.error('Closure methods target not found!');
  const normalizedOld = oldClosureMethodsLoop.replace(/\r\n/g, '\n');
  const normalizedContent = content.replace(/\r\n/g, '\n');
  if (normalizedContent.includes(normalizedOld)) {
     content = normalizedContent.replace(normalizedOld, newClosureMethodsLoop.replace(/\r\n/g, '\n'));
     console.log('Closure methods updated with normalization');
  }
}
